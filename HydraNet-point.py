...
class HydraNet(nn.Module):
    def __init__(self, num_matches, num_sets, num_games, game_offset, feature_dim=16, hydra_dim=16, hidden_dim=64,
        ...
        # Memory management
        self.register_buffer('global_memory_p1', torch.zeros(memory_dim))  # [memory_dim]
        self.register_buffer('global_memory_p2', torch.zeros(memory_dim))  # [memory_dim]
    ...
    def forward(self, batch_dict):
        ...
        for b in range(B):
            ...
            for s in unique_sets:
                ...
                unique_games = torch.unique(game_no[set_mask]).tolist()
                unique_games = sorted(unique_games)

                momentum_p1 = self.set_linear_p1(momentum_p1)
                momentum_p2 = self.set_linear_p2(momentum_p2)
                ...
                for g in unique_games:
                    ...
                    momentum_p1 = last_ball_p1.squeeze(0).detach()  # [16], detached from the computation graph
                    momentum_p2 = last_ball_p2.squeeze(0).detach()  # [16], detached from the computation graph
                    ...
                    # For p1
                    serve_features_p1 = y_p1[:, :, :6]  # [1, L, 6]
                    return_features_p1 = y_p1[:, :, 6:8]  # [1, L, 2]
                    action_features_p1 = y_p1[:, :, 8:15]  # [1, L, 7]
                    pressure_features_p1 = y_p1[:, :, 15:16]  # [1, L, 1]

                    # For p2
                    serve_features_p2 = y_p2[:, :, :6]  # [1, L, 6]
                    return_features_p2 = y_p2[:, :, 6:8]  # [1, L, 2]
                    action_features_p2 = y_p2[:, :, 8:15]  # [1, L, 7]
                    pressure_features_p2 = y_p2[:, :, 15:16]  # [1, L, 1]

                    # Process feature groups with residual connections for p1
                    F1_p1 = self.serve_net_p1(serve_features_p1)  # [1, L, 64]
                    F2_p1 = self.return_net_p1(return_features_p1)  # [1, L, 64]
                    F3_p1 = self.action_net_p1(action_features_p1)  # [1, L, 64]
                    F4_p1 = self.pressure_net_p1(pressure_features_p1)  # [1, L, 64]

                    # Process feature groups with residual connections for p2
                    F1_p2 = self.serve_net_p2(serve_features_p2)  # [1, L, 64]
                    F2_p2 = self.return_net_p2(return_features_p2)  # [1, L, 64]
                    F3_p2 = self.action_net_p2(action_features_p2)  # [1, L, 64]
                    F4_p2 = self.pressure_net_p2(pressure_features_p2)  # [1, L, 64]

                    # Stack features for p1 and p2
                    grouped_features_p1 = torch.stack([F1_p1, F2_p1, F3_p1, F4_p1], dim=2)  # [1, L, 4, 64]
                    grouped_features_p2 = torch.stack([F1_p2, F2_p2, F3_p2, F4_p2], dim=2)  # [1, L, 4, 64]
                    ...
                    for t in range(L):
                        ...
                        # Collect output and loss
                        all_outputs.append(output)
                        all_versus_losses.append(ver_loss)

        # Aggregate outputs and losses for the batch
        if all_outputs:
            # Concatenate all outputs: [1, Total Games]
            final_output = torch.cat(all_outputs, dim=1)  # [1, Total Games]

            # Average contrastive loss
            total_ver_loss = torch.stack(all_versus_losses).mean()  # scalar
        else:
            # If no outputs collected, return zeros
            final_output = torch.zeros(1, 1).to(features.device)
            total_ver_loss = torch.zeros(1).to(features.device)

        return final_output, total_ver_loss


# Dataset Class with Sequence Handling
...
def collate_fn(batch):
    ...
    labels = torch.stack([item['label'] for item in batch], dim=0)  # [B, L]
...
def train_model(train_loader, model, optimizer, device):
    ...
    for batch_idx, batch in enumerate(tqdm(train_loader, desc="Training")):
        ...
        outputs, ver_loss = model(batch)  # outputs: [B, Total Games], ver_loss: scalar

        ver_loss = ver_loss.detach()

        cla_loss = nn.BCEWithLogitsLoss()(outputs, batch['label'])  # [B, L]
    ...
    if predictions.shape != labels.shape:
        # Assuming that labels are per game, flatten them
        labels = labels.flatten()

    predictions_binary = (predictions > 0.0).astype(int)  # BCEWithLogitsLoss doesn't require thresholding at 0.5
...
def main():
    ...
    if not set(df['Y'].unique()).issubset({0, 1}):
        raise ValueError("label 'Y' must only include 0 or 1。")

    # Calculate positive and negative samples
    positive_samples = (df['Y'] == 1).sum()
    negative_samples = (df['Y'] == 0).sum()
    ...
    for fold, (train_match_idx, val_match_idx) in enumerate(match_folds, 1):
        ...
        print(
            f"Fold {fold}: Number of training matches: {len(train_dataset)}, Number of validation matches: {len(val_dataset)}")
...
if __name__ == "__main__":
    log_filename = f"HydraNet-point.txt"
