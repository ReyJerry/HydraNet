    ...
    def forward(self, batch_dict):
        ...
        for b in range(B):
            ...
            for s in unique_sets:
                ...
                last_game_in_set = unique_games[-1]
                ...
                for g in unique_games:
                    # Skip prediction for the last game of the last set
                    if s == last_set and g == last_game_no:
                        print(f"Skipping game {g} in set {s} for Match ID: {match_id}")
                        continue
                    ...
                    grouped_features_p1 = torch.stack([F1_p1, F2_p1, F3_p1, F4_p1], dim=2)  # [1, L_g, 3, 64]
                    grouped_features_p2 = torch.stack([F1_p2, F2_p2, F3_p2, F4_p2], dim=2)  # [1, L_g, 3, 64]

                    features_p1_list = torch.unbind(grouped_features_p1, dim=1)
                    features_p2_list = torch.unbind(grouped_features_p2, dim=1)

                    features_p1_pre_list = torch.unbind(y_p1, dim=1)
                    features_p2_pre_list = torch.unbind(y_p2, dim=1)
                    ...
                    for t in range(L):
                        ...
                        feature_p1_t = features_p1_list[t]  # [1, 3, 64]
                        feature_p2_t = features_p2_list[t]  # [1, 3, 64]
                        ...
                        if g == last_game_in_set and t == L - 1:
                            # Collect the prediction for this game's last point
                            set_predictions.append(output)
                            all_versus_losses.append(ver_loss)

        # After processing all matches in the batch, align predictions with labels
        if set_predictions and labels is not None and len(all_versus_losses) > 0:
            # Convert list of tensors to a single tensor
            predictions_tensor = torch.cat(set_predictions, dim=0)  # [num_predictions, 1]
            ...
            # If no predictions collected, return zeros
            final_output = torch.zeros(1, 1).to(features.device)
            final_labels = torch.zeros(1).to(features.device)
...
class TennisDataset(Dataset):
    def __init__(self, df, match_to_idx, set_offset, game_offset):
        ...
        for match_id, group in grouped:
            ...
            set_victors = group_sorted.groupby(['set_no'])['set_victor'].last().values  # [num_games]

            if len(set_victors) <= 1:
                continue

            # Shift labels by 1 to align predictions with next game's outcome
...
def train_model(train_loader, model, optimizer, device):
    ...
    for batch_idx, batch in enumerate(tqdm(train_loader, desc="Training")):
        ...
        cla_loss = nn.BCEWithLogitsLoss()(outputs, target_labels)  # [B, num_predictions]
    ...
    if predictions and labels:
        ...
        assert len(predictions) == len(
            labels), f"Mismatch in number of predictions and labels: {len(predictions)} vs {len(labels)}"
    else:
...
def main():
    ...
    log_file = open('HydraNet-set.txt', 'w')  # Open the log file for writing
